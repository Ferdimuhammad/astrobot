...
╰─────────❉
_____••••••••• 
(MAAF JIKA ADA FITUR ERROR TANYA AMA OWNER BOTNYA`.trim(),
                        "footerText": "LISSBOT",
  ...
  "title":  "|🙇‍♂️| ❯╾group╼-",
                                ...
                                "rows": [{
                                    "title": ` Semua Perinta dari lissboth`,
...
"title": "「 30 」",
    ...
    await conn.send2ButtonLoc(m.chat, await (await fetch(thumbfoto)).buffer(), text.trim(), watermark, 'JOIN GC', `.? gcbot`, 'DONASI BOT', `.? donasi`, m)
  } catch (e) {
    conn.reply(m.chat, 'Maaf, menu sedang error', m)
    throw e
  }
}
handler.help = ['menu', 'help', '?', '? all']
handler.tags = ['main']
handler.command = /^(menu|help|\?|\? all)$/i
handler.owner = false
handler.mods = false
...
function ucapan() {
  ...
  res = "woy dah malam tidur gays"
  if (time >= 4) {
    res = "*SELAMAT PAGI BRADER*"
  }
  if (time > 10) {
    res = "*SELAMAT SIANG BRADER*"
  }
  if (time >= 15) {
    res = "*SELAMAT SORE BRADER*"
  }
  if (time >= 18) {
    res = "SELAMAT MALAM BRADER*"
  }
  return res
}
