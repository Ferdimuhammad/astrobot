    ...
    try {
      ...
      try {
        ...
        if (user) {
          ...
          exp: 30, //loh ganti gak papa
          limit: 10, //loh ganti gak papa
          registered: false,
          lastclaim: 0, 
        ...
        case 'remove':
          if (chat.welcome) {
            ...
            for (let user of participants) {
              let kai = await(await fetch('https://telegra.ph/file/24d27758ba37286c00288.jpg')).buffer()
              let poi = await(await fetch('https://telegra.ph/file/24d27758ba37286c00288.jpg')).buffer()
          ...
          break
      case 'promote':
        text = (chat.sPromote || this.spromote || conn.spromote || '@user *admin* dia')
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user dia bukan *admin*')
...
DETEKSI @${m.participant.split`@`[0]} HAYO LOH HAPUS APA
...
`.trim(), watermark, 'Matikan Antidelete', '.off delete', m.message)
  ...
  async GroupUpdate({ jid, desc, descId, descTime, descOwner, announce }) {
    ...
    if (!desc) return
    let caption = `
    @${descOwner.split`@`[0]} *DIA TELAH MENGUBAH DESCRIPSI GROUP*.
    ${desc}
        `.trim()
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: '!! KUSUS *OWNER* TOD!!', //ganti terserah loh
    owner: '!! KUSUS *OWNER* TOD!!', //ganti terserah loh
    premium: '!! MAAF FITUR INI KUSUS !! *PREMIUM* !! UNTUK MEMBELI PREMIUM TANYA OWNER KETIK .OWNER', //terserah loh ganti atau gak
    group: ' !! *PERINTAH INI KHUSUS DALAM GROUP* !!', //ganti terserah loh
    private: ' !! *FITUR INI KUSUS DI DALAM CHAT PRIBADI* !!', //ganti tereerah loh
    admin: '!! FITUR INI KUSUS *ADMIN GROUP* !!', //ganti terserah loh
    botAdmin: '!! SILAHKAN JADI KAN BOT *ADMIN* UNTUK PERINTAH INI', //ganti terswrah loh
    unreg: ' *WADUH ANDA BELUM DAFTAR DI DALAM DATABES BOT DAFTAR AJA KETIK*\n\n #daftar nama.umur contohnya \n\n#daftar Dhavin Boyz.19',
    nsfw: 'FITUR INI *NSFW* BELUM AKTIF OY.'
  }[type]
